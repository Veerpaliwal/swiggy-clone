...
export default function Foods() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10">
      ...
      <div className="flex overflow-x-auto gap-10 pb-6 scrollbar-hide">
        {foods.map((food) => (
          <div key={food.id} className="flex-shrink-0 text-center min-w-[140px] md:min-w-[180px]">
            <img
              src={food.image}
              alt={food.name}
              className="w-36 h-36 md:w-44 md:h-44 object-contain hover:scale-110 transition duration-300"
            />
            <p className="mt-4 text-lg font-semibold text-gray-800">{food.name}</p>
          </div>
        ))}
      </div>
    </div>
